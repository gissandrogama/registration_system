defmodule App.Accounts.Adm do
  ...
  """
  def registration_changeset(adm, attrs) do
    adm
  ...
  @doc """
  A adm changeset for changing the e-mail.

  It requires the e-mail to change otherwise an error is added.
  """
  def email_changeset(adm, attrs) do
    adm
    |> cast(attrs, [:email])
    |> validate_email()
    |> case do
  ...
  @doc """
  A adm changeset for changing the password.
  """
  def password_changeset(adm, attrs) do
    adm
    |> cast(attrs, [:password])
  ...
  """
  def confirm_changeset(adm) do
    now = NaiveDateTime.utc_now() |> NaiveDateTime.truncate(:second)
    change(adm, confirmed_at: now)
  end
  ...
  """
  def valid_password?(%App.Accounts.Adm{hashed_password: hashed_password}, password)
