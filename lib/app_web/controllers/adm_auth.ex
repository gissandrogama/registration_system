defmodule AppWeb.AdmAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "adm_remember_me"
  ...
  def log_in_adm(conn, adm, params \\ %{}) do
    ...
    |> redirect(to: adm_return_to || signed_in_path(conn))
  end
  ...
  def log_out_adm(conn) do
    ...
    adm_token && Accounts.delete_session_token(adm_token)
  ...
  def fetch_current_adm(conn, _opts) do
    ...
    adm = adm_token && Accounts.get_adm_by_session_token(adm_token)
  ...
  If you want to enforce the adm e-mail is confirmed before
  ...
  defp maybe_store_return_to(%{method: "GET", request_path: request_path} = conn) do
    put_session(conn, :adm_return_to, request_path)
  end

  defp maybe_store_return_to(conn), do: conn

  defp signed_in_path(_conn), do: "/admins/panel"
end
